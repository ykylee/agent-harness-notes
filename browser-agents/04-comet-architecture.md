...
| 인식 | 접근성 트리 (YAML) | 문서에 미공개 |
| 동작 | 픽셀 좌표 배치 + 고수준 RPC | 문서에 미공개 |
| 자격증명 | **URL 차단**으로 격리 | **값을 안 보여주는** 금고 |
| 전송 | SSE + WebSocket 이중 | 문서에 미공개 |
| 개발자 표면 | 공개 없음 | **CLI · MCP · REPL** |

> ⚠️ 이 표의 Aside 열에 "미공개"가 많다는 것 자체가 사실이다. Comet 은 **리버싱 당했기 때문에**
> 내부가 알려졌고, Aside 는 그런 공개 분석이 아직 없다. **정보량의 차이를 제품 성숙도의 차이로
> 읽으면 안 된다.**
