...
속을 열면 Comet 은 **확장 3개로 구현된 브라우저**다. 겉 분류와 속 구조가 어긋나는 사례로서
가치가 있다.

> ❌ **정정 (2026-09-23)**: 이 문단은 원래 "Aside 와 **정반대 구조**"라고 적었다. **틀렸다.**
> 이후 Aside 브라우저 바이너리를 열어보니 Aside 의 에이전트도 **MV3 크롬 확장**
> (`Aside Browsing Agent`)이었다 — [09 §3](09-aside-browser-internals.md).
> 두 제품은 정반대가 아니라 **같은 구조적 선택**을 했다. 실제 차이는 외피가 아니라
> **계획이 어디서 도는가**다 (Comet 서버 / Aside 로컬 데몬).
> 틀린 서술을 지우지 않고 판정과 함께 남긴다.
...
| 축 | Comet | Aside |
|---|---|---|
| 제어 표면 | 네이티브 브라우저 + **확장 3종** | **Chromium 포크 + 확장 3종** (같다) |
| 계획 위치 | **서버 (Perplexity 백엔드)** | **로컬 데몬 `127.0.0.1:21420` (353MB)** — 구조로 확인 |
...
| 자격증명 | **URL 차단**으로 격리 | **libsodium 금고** (Argon2id/XChaCha20/sealed box) |
