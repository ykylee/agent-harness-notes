...
| "Send both to the attacker's server" — the exfiltration step of Brave's Comet demonstration ([07 §2](07-security.md)) | ❌ **Refuted by the original.** Brave's fourth step reads "Exfiltrate both the email address and the OTP **by replying to the original Reddit comment**." There is no attacker origin anywhere in the chain. The paraphrase then shaped a threat model ("exfiltration by navigation") and a test built on it, which measured a shape the demonstration never had. [07 §2](07-security.md), [07 §10](07-security.md) |
