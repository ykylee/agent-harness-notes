...
| 메모리가 **"plain markdown"** 으로 저장돼 편집 가능 | ⚠️ 2차 출처만 주장. **헬프 문서는 파일 형식도 디스크 경로도 명시하지 않는다** |
...
| **플랫폼 요구사항** | ❌ **문서 내부 불일치** — `get-started.md` 는 "macOS 15.0 or later" 만 말하는데 변경로그는 Windows 정식 지원(v1.0.914.1)을 기록. get-started 가 낡은 것으로 보이나 판정 보류 |
...
| 인식·동작 방식, 전송 계층 | ⚠️ **문서에 없음.** Comet 과 달리 공개 리버싱 분석도 없다 |

> 📌 마지막 항목이 중요하다. [04 §10](04-comet-architecture.md) 의 비교표에서 Aside 열에
> "미공개"가 많은 것은 **제품이 미성숙해서가 아니라 아무도 뜯어보지 않았기 때문**이다.
> 정보량 차이를 성숙도 차이로 읽으면 안 된다.
...
| 항목 | 이유 |
|---|---|
| **제품을 직접 실행하지 않았다** | 실제 UI, 애니메이션, 진행 상태 표현, 오류 화면, 승인 모달의 생김새는 확인 불가. [03](03-aside-design-ux.md) 은 **문서가 규정한 상호작용 모델**에 한정된다 |
...
- [ ] Aside 를 실제 설치해 UI/UX 를 1차로 확인 (특히 승인 흐름, 진행 상태 표현)
- [ ] `aside repl` 과 `aside mcp` 의 실제 표면 확인 — 이 제품에서 구조적으로 가장 흥미로운 부분
